<template>
  ...
  <div class="admin-page">
    <PageHeaderBar title="学生管理" subtitle="管理学校学生用户信息">
      <template #actions>
      ...
      <a-row :gutter="16">
        ...
        <a-col :span="18">
          <a-card :bordered="false">
            ...
            >
              <template #emptyText>
                <EmptyStateBlock description="暂无学生数据" />
              </template>
              <!-- 操作列 -->
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'action'">
                  <a-space>
                    ...
                    <a-popconfirm
                      title="删除后将清除该用户学习记录及优秀作业等数据，是否确定删除该用户?"
                      ok-text="确定"
                      cancel-text="取消"
    ...
    <a-modal
      ...
      cancel-text="取消"
    >
      <p>删除后将清除所选用户学习记录及优秀作业等数据，是否确定删除所选用户?</p>
    </a-modal>
...
const loadTableData = async () => {
  ...
  try {
    ...
    const result = await studentStore.getStudentList(params);
...
// 添加学生
const handleAdd = () => {
  router.push('/admin/user/student/add');
};

// 编辑学生
const handleEdit = (record: any) => {
  router.push(`/admin/user/student/edit/${record.id}`);
};

// 删除学生
const handleDelete = async (record: any) => {
  try {
    await studentStore.deleteStudent(record.id);
...
const downloadTemplate = () => {
  message.info('正在下载模板...');
  window.open('/api/v1/system/students/export/template', '_blank');
};
...
const handleImportOk = async () => {
  ...
  try {
    ...
    const result = await studentStore.importStudentsFromFile(file);
    ...
    if (result) {
      ...
      if (result.error_count > 0) {
        message.warning(`${result.error_count} 名学生导入失败`);
      }
    }
...
const confirmBatchDelete = async () => {
  try {
    await studentStore.batchDeleteStudents(selectedRowKeys.value);
...
const confirmStatusChange = async () => {
  try {
    ...
    await studentStore.batchUpdateStudentStatus(selectedRowKeys.value, status);
...
const handleTransferOk = async () => {
  ...
  try {
    ...
    await studentStore.transferStudents(selectedRowKeys.value, transferTargetId.value);
