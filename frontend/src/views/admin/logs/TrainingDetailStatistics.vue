<template>
  <div class="statistics-container">
    <div class="page-header">
      <h2 class="page-title">
        {{ trainingName }} <span v-if="timeRangeLabel">({{ timeRangeLabel }})</span>
...
// 返回实训列表页
const goBack = () => {
  router.push('/admin/logs/training');
};
...
const fetchData = async (sorter?: any) => {
  ...
  try {
    ...
    // 请求数据
    const response = await fetch(`/v1/admin/logs/training/detail/${trainingId.value}?${new URLSearchParams(params)}`);
    ...
    if (result.code === 200) {
      ...
      // 更新实训名称
      if (result.data.trainingName) {
        trainingName.value = result.data.trainingName;
      }
    } else {
...
// 导出数据
const exportData = () => {
  message.success(`已导出${trainingName.value}${timeRangeLabel.value ? '（' + timeRangeLabel.value + '）' : ''}的统计数据`);
};
