  ...
  <div class="admin-page">
    ...
    <div class="practice-course-container">
      <a-row :gutter="16">
        <!-- 左侧分类树 -->
        <a-col :span="6">
          <a-card title="课程分类" :bordered="false" class="category-tree-card">
            <a-spin :spinning="loading.practiceCourseCategories">
              <a-tree
                v-model:expandedKeys="expandedKeys"
                v-model:selectedKeys="selectedKeys"
                :tree-data="practiceCourseCategories"
        ...
        <a-col :span="18">
          <a-card :bordered="false" class="course-list-card">
            ...
            <a-table
              ...
              :dataSource="filteredPracticeCourses"
              :columns="columns"
...
const practiceCourseCategories = computed(() => adminCourseStore.practiceCourseCategories);
const filteredPracticeCourses = computed(() => adminCourseStore.filteredPracticeCourses);
...
const columns = [
  ...
  },
  {
    ...
    dataIndex: 'title',
    ellipsis: true,
    sorter: (a: PracticeCourse, b: PracticeCourse) => a.title.localeCompare(b.title),
  },
  {
    title: '教师',
    dataIndex: 'teacher',
    width: 120,
    sorter: (a: PracticeCourse, b: PracticeCourse) => a.teacher.localeCompare(b.teacher),
  },
  {
    title: '所属院校',
  ...
  },
  {
    ...
    onFilter: (value: string, record: PracticeCourse) => record.status === value,
  },
  {
    title: '更新时间',
    dataIndex: 'updatedAt',
    width: 170,
    sorter: (a: PracticeCourse, b: PracticeCourse) =>
...
// 页面加载时获取数据
onMounted(async () => {
  if (practiceCourseCategories.value.length === 0) {
    await adminCourseStore.fetchPracticeCourseCategories();
  }
  await adminCourseStore.fetchPracticeCourses();
});

// 选择分类
const onSelectCategory = (selectedKeys: string[]) => {
  if (selectedKeys.length > 0) {
    adminCourseStore.setFilter('practice', 'categoryKey', selectedKeys[0]);
  }
};

// 搜索
const onSearch = () => {
  adminCourseStore.setFilter('practice', 'keyword', keyword.value);
};
...
const resetFilters = () => {
  ...
  adminCourseStore.resetFilters('practice');
};

// 查看课程详情
const viewCourse = (course: PracticeCourse) => {
  router.push(`/admin/course/detail/${course.id}`);
};

// 下架课程确认
const showUnpublishConfirm = (course: PracticeCourse) => {
  Modal.confirm({
    title: '确认下架课程',
...
// 发布课程确认
const showPublishConfirm = (course: PracticeCourse) => {
  Modal.confirm({
    title: '确认发布课程',
