    ...
    <div class="question-form">
      <a-card :loading="loading">
        ...
        <div class="type-select-container">
          <a-radio-group v-model:value="currentType" button-style="solid" disabled>
...
// 试题ID (支持字符串和数字)
const questionId = ref<string | number>('');
...
const formState = reactive({
  ...
  options: [] as Array<{ key: string; content: string }>,
...
const loadQuestionDetail = async () => {
  if (!userStore.userInfo?.id || !questionId.value) return;
  
  loading.value = true;
  try {
    const response = await getQuestionDetail(questionId.value, userStore.userInfo.id);
    
    if (response.code === '0000') {
      const question = response.data;
      
      // 设置题型
      currentType.value = question.question_type;
      
      // 填充基本信息
      formState.content = question.content;
      formState.direction = question.direction || undefined;
      ...
      if (question.question_type === 'SINGLE_CHOICE' || question.question_type === 'MULTIPLE_CHOICE') {
        formState.options = question.options || [];
        formState.correct_answers = question.correct_answers || [];
      } else if (question.question_type === 'TRUE_FALSE') {
        formState.judgeAnswer = question.correct_answers?.[0] || 'true';
      } else if (question.question_type === 'SHORT_ANSWER') {
        formState.essayAnswer = question.correct_answers?.[0] || '';
      }
    } else {
      message.error(response.message || '加载试题详情失败');
      router.back();
    }
  } catch (error) {
    console.error('加载试题详情失败:', error);
    message.error('加载试题详情失败');
    router.back();
  } finally {
    loading.value = false;
  }
...
const saveQuestion = async () => {
  if (!userStore.userInfo?.id || !questionId.value) {
  ...
  try {
    ...
    // 构建请求数据
    const questionData: Partial<CreateQuestionRequest> = {
    ...
    saving.value = true;
    const response = await updateQuestion(questionId.value, questionData, userStore.userInfo.id);
    ...
    if (response.code === '0000') {
      ...
      message.error(response.message || '更新试题失败');
    }
  } catch (error) {
...
onMounted(() => {
  ...
  if (id) {
    questionId.value = id as string;
    loadQuestionDetail();
  } else {
    message.error('缺少试题ID');
    router.back();
  }
});
</script>
