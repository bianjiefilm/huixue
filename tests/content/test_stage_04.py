...
Red phase: all tests should FAIL on the current output/stage_04_string.json.
...
JSON_PATH = PROJECT_ROOT / "output" / "stage_04_string.json"
YAML_PATH = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "python" / "stage_4.yaml"
...
class TestTestCases:
    ...
    def test_boundary_case_exists(self, test_cases: list[dict]):
        """At least 1 test case must be a boundary case: empty string, or len=1."""
        BOUNDARY_VALUES = {"", "0", "1", "a"}
