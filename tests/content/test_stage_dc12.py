"""
Stage DC12 (综合项目与评估) TDD tests.
Red phase: all tests should FAIL on the current output/stage_12_comprehensive_project.json.
...
JSON_PATH = PROJECT_ROOT / "output" / "stage_12_comprehensive_project.json"
YAML_PATH = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "data_collection" / "stage_12.yaml"
...
def raw_json() -> dict:
    """Load and parse the stage 12 JSON file."""
...
def stage_config(raw_json: dict) -> dict:
    """Parse stage_12.yaml expectations."""
    import yaml
...
class TestTestCases:
    ...
    def test_expected_output_no_trailing_newline(self, test_cases: list[dict]):
        ...
        for tc in test_cases:
            exp = tc["expected"]
            exp_str = json.dumps(exp, ensure_ascii=False) if not isinstance(exp, str) else exp
            if exp_str.endswith("\n"):
                failures.append(
