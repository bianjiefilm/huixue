"""
Stage 6 (字典与集合) TDD tests.
Red phase: all tests should FAIL on the current output/stage_06_dict_set.json.
...
JSON_PATH = PROJECT_ROOT / "output" / "stage_06_dict_set.json"
YAML_PATH = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "python" / "stage_6.yaml"
...
def raw_json() -> dict:
    """Load and parse the stage 6 JSON file."""
...
def stage_config(raw_json: dict) -> dict:
    """Parse stage_6.yaml expectations."""
    import yaml
...
class TestTestCases:
    ...
    def test_boundary_case_exists(self, test_cases: list[dict]):
        """At least 1 test case must be a boundary case: empty/single/short input."""
        BOUNDARY_VALUES = {"", "a", "a b", "one one one"}
