"""
Stage 7 (文件操作与异常处理) TDD tests.
Red phase: all tests should FAIL on the current output/stage_07_file_exception.json.
...
JSON_PATH = PROJECT_ROOT / "output" / "stage_07_file_exception.json"
YAML_PATH = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "python" / "stage_7.yaml"
...
def raw_json() -> dict:
    """Load and parse the stage 7 JSON file."""
...
def stage_config(raw_json: dict) -> dict:
    """Parse stage_7.yaml expectations."""
    import yaml
...
class TestTestCases:
    ...
    def test_boundary_case_exists(self, test_cases: list[dict]):
        ...
        BOUNDARY_VALUES = {"p:1 ", "x:5,y:3 x:1,z:9", ""}
