"""
Stage BD-1 (Hadoop概述与集群搭建) TDD tests.
Red phase: all tests should FAIL on current output/stage_bigdata_01-06.json.
...
JSON_FILE = PROJECT_ROOT / "output" / "stage_bigdata_01-06.json"
YAML_FILE = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "bigdata" / "stage_1.yaml"
...
def raw_json() -> dict:
    ...
    with open(JSON_FILE, encoding="utf-8") as f:
        data = json.load(f)
    # Support both single-stage and stages-array formats
    if "stages" in data:
        for s in data["stages"]:
            if s.get("task_id") == 1:
                return s
        pytest.fail("task_id=1 not found in stages array")
    return data
...
def stage_config() -> dict:
    import yaml
    assert YAML_FILE.exists(), f"YAML not found: {YAML_FILE}"
...
class TestBasicStructure:
    ...
    def test_handbook_min_chars(self, handbook: str, stage_config: dict):
        ...
        actual = len(handbook)
        assert actual >= min_chars, f"Handbook {actual} chars < {min_chars} required"
    ...
    def test_questions_count(self, questions: list[dict], stage_config: dict):
        ...
        assert len(questions) >= expected - 1, f"Questions {len(questions)} < {expected-1}"
    ...
    def test_visible_test_cases(self, test_cases: list[dict], stage_config: dict):
        ...
        assert len(visible) == stage_config["expected_test_cases_visible"], \
            f"Visible: {len(visible)}, expected: {stage_config['expected_test_cases_visible']}"
    ...
    def test_hidden_test_cases(self, test_cases: list[dict], stage_config: dict):
        ...
        assert len(hidden) == stage_config["expected_test_cases_hidden"], \
            f"Hidden: {len(hidden)}, expected: {stage_config['expected_test_cases_hidden']}"
...
class TestContentConsistency:
    ...
    def test_baseline_code_incomplete(self, baseline_code: str):
        if not baseline_code:
            pytest.skip("No baseline_code provided")
        try:
            ast.parse(baseline_code)
        except SyntaxError:
            pytest.fail(f"baseline_code has SyntaxError")
        ...
        try:
            ...
            pass
        assert has_marker or has_stub, "baseline_code appears complete (no TODO/FIXME, no stub)"
...
class TestTestCases:
    def test_visibility_split(self, test_cases: list[dict]):
        ...
        # First two visible, last two hidden
    ...
    def test_boundary_case_exists(self, test_cases: list[dict]):
        ...
        assert has_boundary, f"No boundary case. Inputs: {[tc['input'] for tc in test_cases]}"
