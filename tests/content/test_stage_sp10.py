"""
Stage SP-10 (Structured Streaming) TDD tests.
"""
...
JSON_FILE = PROJECT_ROOT / "output" / "stage_spark_07-12.json"
YAML_FILE = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "spark" / "stage_10.yaml"
...
def raw_json() -> dict:
    ...
    if "stages" in data:
        for s in data["stages"]:
            if s.get("task_id") == 10:
                return s
        pytest.fail("task_id=10 not found")
    return data
