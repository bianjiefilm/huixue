"""
Stage DC01 (数据采集概述与工具选型) TDD tests.
Red phase: all tests should FAIL on the current output/stage_1_data_collection_overview.json.
...
JSON_PATH = PROJECT_ROOT / "output" / "stage_1_data_collection_overview.json"
YAML_PATH = PROJECT_ROOT / "content_orchestrator" / "stages_config" / "data_collection" / "stage_1.yaml"
...
def raw_json() -> dict:
    """Load and parse the stage 1 JSON file."""
...
def stage_config(raw_json: dict) -> dict:
    """Parse stage_1.yaml expectations."""
    import yaml
...
def test_cases(raw_json: dict) -> list[dict]:
    # Support both top-level test_cases and question_data.test_cases
...
class TestContentConsistency:
    ...
    def test_baseline_code_fails_all_test_cases(
        ...
        for tc in test_cases:
            ...
            # Escape newlines for shell safety
            inp_safe = inp.replace("'", "'\"'\"'")
            code_safe = baseline_code.replace("'", "'\"'\"'")
            script = f"code={code_safe}\nresult={inp_safe!r}\nexec(code)"
            try:
