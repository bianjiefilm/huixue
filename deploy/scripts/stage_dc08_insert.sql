-- ============================================================
-- Stage 8: 定时任务调度
-- practice_id=4, order_in_practice=8
-- ============================================================
...
BEGIN
    ...
    INSERT INTO task_tests (task_id, case_id, input_data, expected_output, is_hidden, description, match_rule, test_order) VALUES
        (new_task_id, 'tc1', $s8$配置每小时执行一次采集任务$s8$, $s8$BlockingScheduler with interval(hours=1)$s8$, false, $s8$配置每小时执行一次采集任务$s8$, 'CONTAINS', 1),
        (new_task_id, 'tc2', $s8$配置每天凌晨2点执行采集任务$s8$, $s8$BlockingScheduler with cron trigger at hour=2$s8$, false, $s8$配置每天凌晨2点执行采集任务$s8$, 'CONTAINS', 2),
        (new_task_id, 'tc3', $s8$配置每30分钟执行一次$s8$, $s8$BlockingScheduler with interval(minutes=30)$s8$, true, $s8$配置每30分钟执行一次$s8$, 'CONTAINS', 3),
        (new_task_id, 'tc4', $s8$配置每周一凌晨执行$s8$, $s8$BlockingScheduler with cron day_of_week=''mon''$s8$, true, $s8$配置每周一凌晨执行$s8$, 'CONTAINS', 4),
        (new_task_id, 'tc5', $s8$配置每天上午9点到下午6点每小时执行$s8$, $s8$BlockingScheduler with cron hour=''9-17''$s8$, true, $s8$配置每天上午9点到下午6点每小时执行$s8$, 'CONTAINS', 5),
        (new_task_id, 'tc6', $s8$配置每月1号凌晨执行$s8$, $s8$BlockingScheduler with cron day=1$s8$, true, $s8$配置每月1号凌晨执行$s8$, 'CONTAINS', 6),
        (new_task_id, 'tc7', $s8$配置工作日上午9:30执行$s8$, $s8$BlockingScheduler with cron hour=9, minute=30, day_of_week=''mon-fri''$s8$, true, $s8$配置工作日上午9:30执行$s8$, 'CONTAINS', 7),
        (new_task_id, 'tc8', $s8$配置每5分钟执行一次采集$s8$, $s8$BlockingScheduler with interval(minutes=5)$s8$, true, $s8$配置每5分钟执行一次采集$s8$, 'CONTAINS', 8),
        (new_task_id, 'tc9', $s8$配置每周日凌晨1点清理缓存$s8$, $s8$BlockingScheduler with cron day_of_week=''sun'', hour=1$s8$, true, $s8$配置每周日凌晨1点清理缓存$s8$, 'CONTAINS', 9),
        (new_task_id, 'tc10', $s8$配置每天0点执行日报生成$s8$, $s8$BlockingScheduler with cron hour=0, minute=0$s8$, true, $s8$配置每天0点执行日报生成$s8$, 'CONTAINS', 10)
;
